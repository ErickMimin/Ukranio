...
SocketDatagrama::SocketDatagrama(int pto){
	...
	direccionLocal.sin_family= AF_INET; //Familia de dirección
	direccionLocal.sin_addr.s_addr= INADDR_ANY; //Poner la dirección IP del servidor automáticamente
	direccionLocal.sin_port= htons(pto); //Se decide el puerto, si es 0 entonces se asignará automáticamente
	bind(s,(struct sockaddr *)&direccionLocal, sizeof(direccionLocal)); //asocia un socket a un puerto local
}

SocketDatagrama::~SocketDatagrama(){
	close(s); //Cerramos la conexión de nuestro socket
}
...
int SocketDatagrama::recibe(PaqueteDatagrama & p){
    ...
    socklen_t clilen = sizeof(direccionForanea); //Variable que almacena la longitud de la dirección foranea
    ...
    /*S --> Descriptor del socket por el cual se leen los datos,
      datos --> Buffer donde se almacena la información,
      ...
      */
    inet_ntop(AF_INET, &(direccionForanea.sin_addr), dire, INET_ADDRSTRLEN);//Convertimos la dirección en una cadena adecuada para su lectura
    p.inicializaPuerto(ntohs(direccionForanea.sin_port)); //Iniciamos el puerto
    p.inicializaIp(dire); //Inicializamos la dirección IP
